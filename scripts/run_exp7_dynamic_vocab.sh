#!/usr/bin/env bash
# Exp 7: Episode-local dynamic vocabulary expansion
#
# Core idea: for each training episode, expand the action space beyond the base
# 8000 words to include: (a) the title word(s) for that episode, and (b) their
# top-20 TinyModel nearest-neighbors.  This means title words ARE in the action
# space during training → the model can actually solve episodes → GRPO sees real
# +1000 reward signals instead of always-negative rewards.
#
# At inference time, the expansion uses TinyModel neighbors of high-similarity
# revealed words (test-time compatible, no title leakage).
#
# Base model: v7 frozen backbone (fast iteration, clean ablation vs exp6).
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-exp7_dynamic_vocab}"
