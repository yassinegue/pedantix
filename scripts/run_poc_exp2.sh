#!/usr/bin/env bash
# Exp 2: title-word logit-bias warm start (+2.0 on title-only indices)
# Hypothesis: boosting title-word sampling probability ~7x causes title hits within 30 steps
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-poc_exp2_frozen_200_bias}"
